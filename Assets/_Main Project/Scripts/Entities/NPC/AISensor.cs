...
public class AISensor : MonoBehaviour {
    ...
    private void Update() {
        ...
        if(targets == null || targets.Count == 0) {
            if (brain.CurrentStateName != "Idle")
                brain.OnNoTargetsLeft();
        }
    }
    ...
    private void OnStatChanged(EventData data) {
        ...
        if (targetStat == StatName.Health && value < 0f) {
            if (detectOnDamageTaken == true && cause != null && cause != owner) {
                if(targets.Contains(cause) == true) {
                    return;
                }
    ...
    private Entity IsDetectionValid(Collider2D other) {
        ...
        if(other == owner.GetComponent<Collider2D>()) {
            Debug.Log("Deteected self: " + owner.EntityName);
            return null;
        }
